...
namespace StockMarket.Adapter {
    public class RdotNetAdapter : IRdotNetAdapter {
        ...
        // new imp 
        public IEnumerable<MACDIndex> CalculateMACDIndex (IEnumerable<TimeSeriesIndex> input) {

            var StockDataFrame = RDotNetConvertor.TimeSeriesToDataFrame (input, engine);
            engine.SetSymbol ("datasets", StockDataFrame);
            engine.Evaluate ($"macd <- MACD(datasets[,'Close'], 12, 26, 9 ,  maType = 'EMA')");
            engine.Evaluate ("datasets$MADC <- (macd[,'macd'])");
            engine.Evaluate ("datasets$Signal <- (macd[,'signal'])");
            var result = engine.Evaluate ("finalReslt <- data.frame(datasets)").AsDataFrame ();
            var final = RDotNetConvertor.DataFrametoMACDIndexMapper (result);

            return final;
        }

        public IEnumerable<RSIIndex> CalculateRSIIndex (IEnumerable<TimeSeriesIndex> input) {
            ...
            var result = engine.Evaluate ("finalReslt <- data.frame(datasets)").AsDataFrame ();
            var final = RDotNetConvertor.DataFrametoRSIIndexMapper (result);

            return final;
        }

        public IEnumerable<SOIndex> CalculateSOIndex (IEnumerable<TimeSeriesIndex> input) {
            ...
            var result = engine.Evaluate ("finalReslt <- data.frame(datasets)").AsDataFrame ();
            var final = RDotNetConvertor.DataFrametoSOIndexMapper (result);
            return final;
        }

        public IEnumerable<GuppyIndex> CalculateGuppyIndex (IEnumerable<TimeSeriesIndex> input) {
            var StockDataFrame = RDotNetConvertor.TimeSeriesToDataFrame (input, engine);
            engine.SetSymbol ("datasets", StockDataFrame);
            ...
            var result = engine.Evaluate ("finalReslt <- data.frame(datasets)").AsDataFrame ();
            var final = RDotNetConvertor.DataFrametoGuppyIndexMapper (result);
            return final;
